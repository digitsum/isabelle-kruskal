...
fun uf_rep_of p i =
  (fn () => let
            ...
            in
              (if equal_nata n i then (fn () => i) else uf_rep_of p n) ()
            end);
...
fun uf_union u i j =
  ...
  in
    (fn () =>
      ...
      in
        (if equal_nata ci cj then (fn () => (s, p))
...
fun kruskal_tree x =
  (fn xi => fn () => let
                       val xa = kruskal xi ();
                     in
                       (if is_connected_list xi xa then SOME xa else NONE)
                     end)
    x;
